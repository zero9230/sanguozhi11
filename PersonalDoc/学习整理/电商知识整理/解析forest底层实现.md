...
每天夜深人静时，这个g物总会掀起一片血雨腥风，饱受摧残的人们苦不堪言，却只能默默忍受。
...
![img](解析forest底层实现.assets/forest-1.png)
...
![image-20211027221857297](解析forest底层实现.assets/forest-2-1.png)
...
![img](解析forest底层实现.assets/forest-3.png)
...
## 2.3 字典表文件(string dictionary)

字典表文件，其实很简单，它存放了本数据包中使用到的所有字符串，比如类目名称、类目属性別名、类目属性值的特征等等。如下图所示。

![img](解析forest底层实现.assets/forest-4.png)
...
![img](解析forest底层实现.assets/forest-5.png)
...
这回先看合影，如下图所示。
 ...
 ![img](解析forest底层实现.assets/forest-6.png) 
...
放上一个比较全面的结构图。
 ...
 ![img](解析forest底层实现.assets/forest-7.png) 
...
![img](解析forest底层实现.assets/forest-8.png)
...
![img](解析forest底层实现.assets/forest-11.png)
...
我们会把一次UPDATE的操作按上图格式存储，其中包含变更修改前与修改后的payload,看到这条Action,修改前是  `{"cat_id":"300","parent_id":"100",...}` , 修改后是 `{"cat_id":"300",l'parent_id":"200",...}` ， 据此我们能够很容易地算出 父类目的变化。针对父类目变化的情况，产生一条DELETE的Relation记录”类目B少了D这个子类目“；再产生一条 ADD的Relation记录"类目C多了 D这个子类目"。
 ...
 ![img](解析forest底层实现.assets/forest-9.png)
...
![img](解析forest底层实现.assets/forest-10.png)
